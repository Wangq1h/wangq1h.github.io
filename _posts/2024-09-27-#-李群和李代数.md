...
* $\mathfrak{sl}$代表零迹矩阵的集合，即$\mathfrak{sl}(n,\kappa)={A\in\mathfrak{gl}(n,\kappa)|\mathrm{tr}(A)=0}$。显然，其封闭性可以由$\mathrm{tr}(AB)=\mathrm{tr}(BA)$保证。
* $\mathfrak{so}$代表反对称矩阵的集合，即$\mathfrak{so}(n,\kappa)={A\in\mathfrak{gl}(n,\kappa)|A^T=-A}$。
* $\mathfrak{u}$的线性空间为反厄米矩阵的集合，即$\mathfrak{u}(n,\kappa)={A\in\mathfrak{gl}(n,\kappa)|A^\dagger=-A}$。
...
$$
\begin{align}
c_*(g)(e)=\frac{\mathrm d}{\mathrm dt}c(g)(e^{t A})|_{t=0}\triangleq\mathrm{Ad}(g)(A)\quad A\in\mathfrak{g}
\end{align}
$$
...
$$
\begin{align}
\frac{\mathrm d}{\mathrm dt}Ad(e^{tB})(A)|_{t=0}=\mathrm{ad}(B)(A)=[B,A]\\
...
$$
\begin{align}
|M|^2=\vec M\cdot \vec M = (x^2+y^2-t^2)\mathbf 1
\end{align}
$$
...
$$
\begin{align}
\mathrm{SO}(3)={R(\mathbf n,\theta)||\mathbf n|=1,0\leq\theta\leq180)}
\end{align}
$$
...
![1722434627900](image/李群和李代数/1722434627900.png "$\mathbf{SO}(3)$群的二维表示")
...
$$
\begin{align}
A_i=-\frac{\mathrm d}{\mathrm d\theta}Q(\mathbf i,\theta)|_{\theta=0}
\end{align}
$$
...
$$
\begin{align}
J|n_1,n_2\rangle=\frac{n_1+n_2}2|n_1,n_2\rangle\\
J_3|n_1,n_2\rangle=\frac{n_1-n_2}2|n_1,n_2\rangle
\end{align}
$$
...
$$
\begin{align}
D^j\otimes D^{j^{\prime}}=D^{j+j^{\prime}}\oplus D^{j+j^{\prime}-1}\oplus\cdots\oplus D^{|j-j^{\prime}|}
\end{align}
$$
